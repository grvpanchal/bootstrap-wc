...
/**
 * <bs-search-select> — wraps the upstream TomSelect library inside a Lit element.
 *
 ...
 * `globalThis.TomSelect` at instantiation time.
 *
 * Common attributes:
 ...
 */
export class BsSearchSelect extends LitElement {
  ...
  /**
   * Create the upstream TomSelect instance against the slotted target element
   * and dispatch `bs:ready` with the instance.
   */
  protected instantiate() {
    const TS = (globalThis as { TomSelect?: new (el: HTMLElement, opts: unknown) => { destroy: () => void } }).TomSelect;
    if (!TS) return console.warn('[bs-search-select] TomSelect global not found.');
    this.target = (this.querySelector('select, input') as HTMLElement) ?? (this.firstElementChild as HTMLElement);
    if (!this.target) return;
    this.instance = new TS(this.target, this.options ?? {});
