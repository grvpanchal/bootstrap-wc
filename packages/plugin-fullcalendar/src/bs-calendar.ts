...
/**
 * <bs-calendar> — wraps the upstream FullCalendar library inside a Lit element.
 *
 ...
 * `globalThis.FullCalendar` at instantiation time.
 *
 * Common attributes:
 ...
 */
export class BsCalendar extends LitElement {
  ...
  /**
   * Create the upstream FullCalendar instance against the slotted target element
   * and dispatch `bs:ready` with the instance.
   */
  protected instantiate() {
    const FC = (globalThis as { FullCalendar?: { Calendar: new (el: HTMLElement, opts: unknown) => { render: () => void; destroy: () => void } } }).FullCalendar;
    if (!FC?.Calendar) {
      console.warn('[bs-calendar] FullCalendar global not found. Load fullcalendar before this element.');
      return;
    }
    this.target = (this.firstElementChild as HTMLElement) ?? this;
    const inst = new FC.Calendar(this.target, this.options ?? {});
    inst.render();
    this.instance = inst;
