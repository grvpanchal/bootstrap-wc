...
/**
 * <bs-file-upload> — wraps the upstream Dropzone library inside a Lit element.
 *
 ...
 * `globalThis.Dropzone` at instantiation time.
 *
 * Common attributes:
 ...
 */
export class BsFileUpload extends LitElement {
  ...
  /**
   * Create the upstream Dropzone instance against the slotted target element
   * and dispatch `bs:ready` with the instance.
   */
  protected instantiate() {
    const Dropzone = (globalThis as { Dropzone?: new (el: HTMLElement, opts: unknown) => { destroy: () => void } }).Dropzone;
    if (!Dropzone) return console.warn('[bs-file-upload] Dropzone global not found.');
    this.target = (this.querySelector('form, .dropzone') as HTMLElement) ?? (this.firstElementChild as HTMLElement);
    if (!this.target) return;
    this.instance = new Dropzone(this.target, this.options ?? {});
