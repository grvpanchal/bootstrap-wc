...
/**
 * <bs-datepicker> — wraps the upstream flatpickr library inside a Lit element.
 *
 ...
 * `globalThis.flatpickr` at instantiation time.
 *
 * Common attributes:
 ...
 */
export class BsDatepicker extends LitElement {
  ...
  /**
   * Create the upstream flatpickr instance against the slotted target element
   * and dispatch `bs:ready` with the instance.
   */
  protected instantiate() {
    const fp = (globalThis as { flatpickr?: (el: HTMLElement, opts: unknown) => { destroy: () => void } }).flatpickr;
    if (!fp) return console.warn('[bs-datepicker] flatpickr global not found.');
    this.target = (this.querySelector('input') as HTMLElement) ?? (this.firstElementChild as HTMLElement);
    if (!this.target) return;
    this.instance = fp(this.target, this.options ?? {});
