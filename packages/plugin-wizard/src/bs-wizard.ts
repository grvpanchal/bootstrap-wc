...
/**
 * <bs-wizard> — wraps the upstream SmartWizard 6 library inside a Lit element.
 *
 ...
 * `globalThis.jQuery.fn.smartWizard` at instantiation time.
 *
 * Common attributes:
 ...
 */
export class BsWizard extends LitElement {
  ...
  /**
   * Create the upstream SmartWizard 6 instance against the slotted target element
   * and dispatch `bs:ready` with the instance.
   */
  protected instantiate() {
    const $ = (globalThis as { jQuery?: ((sel: HTMLElement) => { smartWizard: (opts: unknown) => unknown; data: (k: string) => unknown }) & { fn?: { smartWizard?: unknown } } }).jQuery;
    if (!$?.fn?.smartWizard) return console.warn('[bs-wizard] jQuery + SmartWizard not found.');
    this.target = (this.firstElementChild as HTMLElement) ?? this;
    const wrapper = $(this.target);
    wrapper.smartWizard(this.options ?? {});
    this.instance = wrapper.data('smartWizard');
