...
/**
 * <bs-clipboard> — wraps the upstream clipboard.js library inside a Lit element.
 *
 ...
 * `globalThis.ClipboardJS` at instantiation time.
 *
 * Common attributes:
...
export class BsClipboard extends LitElement {
  ...
  /**
   * Create the upstream clipboard.js instance against the slotted target element
   * and dispatch `bs:ready` with the instance.
   */
  protected instantiate() {
    const ClipboardJS = (globalThis as { ClipboardJS?: new (sel: string | HTMLElement, opts?: unknown) => { destroy: () => void } }).ClipboardJS;
    if (!ClipboardJS) return console.warn('[bs-clipboard] ClipboardJS global not found.');
    this.target = (this.querySelector('[data-clipboard-text], [data-clipboard-target]') as HTMLElement) ?? (this.firstElementChild as HTMLElement) ?? this;
    if (!this.target) return;
    this.instance = new ClipboardJS(this.target, this.options ?? {});
